...
##########################
##### Solution approach 2
#Using Window function LEAD() populate Effective start_dt and end_dt. so the price will be effective in this range.
WITH CTE_EffectiveDateRange AS (
	SELECT propertyId, utilityName, utilityCharge, effectiveDate start_dt, 
	IFNULL(DATE_SUB(LEAD(effectiveDate) OVER(PARTITION BY propertyId, utilityName), INTERVAL 1 DAY), '9999-01-01') AS end_dt
	FROM tblutility
),
#create a monthly break down without charges
CTE_MonthlyBreakdown AS 
(
	SELECT *
	FROM (SELECT DISTINCT propertyId, utilityName
	FROM tblutility) ut, tblDimDate
) 
#we can populate the charges, using co-related sub-queries, where will pull the charge where the month between effective start and end_dt of above cte
SELECT DISTINCT brk.propertyId, brk.utilityName, DATE_FORMAT(brk.dateMonth, '%M %Y') BillingMonth, (
		SELECT utilityCharge FROM CTE_EffectiveDateRange 
		WHERE eff.propertyId = propertyId AND eff.utilityName = utilityName AND brk.dateMonth BETWEEN start_dt AND end_dt
        ) charges
